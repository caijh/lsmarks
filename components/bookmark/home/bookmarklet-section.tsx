...
export function BookmarkletSection() {
  ...
  const instructionsSteps = [
    "将按钮拖动到浏览器的书签栏",
    "浏览网页时，点击书签栏中的\"添加到雷水书签\"",
    "在弹出的窗口中选择收藏集合并保存"
  ];
  const compatibility = "适用于所有主流浏览器";
  const oneClickTitle = "一键收藏，告别复制粘贴";
  const oneClickDescription = "无需复制网址、切换标签页或打开新窗口，只需点击书签栏中的按钮，即可将当前浏览的网页添加到您的雷水书签集合中。";
  const syncTitle = "跨设备同步，随处可用";
  const syncDescription = "所有通过 Bookmarklet 添加的书签都会自动同步到您的雷水书签账户，让您在任何设备上都能访问您的收藏。";
