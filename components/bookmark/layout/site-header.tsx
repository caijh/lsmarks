...
export function SiteHeader({ user }: SiteHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b glass-navbar backdrop-blur-sm shadow-sm">
      ...
      <div className="container flex h-14 sm:h-16 items-center px-1 sm:px-4 justify-between">
        <div className="flex gap-1 sm:gap-6 md:gap-10 items-center overflow-hidden">
          {isCollectionPage && currentCollection ? (
            <>
              {/* 在集合页面只显示集合名称 */}
              <Logo showText={true} className="flex" />
            </>
          ) : (
            <>
              {/* 在其他页面显示正常的Logo */}
        ...
        <div className="flex items-center justify-end ml-auto">
          ...
          {isCollectionPage && currentCollection && user && (
            <>
              {/* 编辑模式下的工具按钮 */}
              {editMode && (
                <div className="flex items-center gap-1 mr-2 bg-background/30 backdrop-blur-sm rounded-full px-2 py-1 border border-border/30 shadow-sm">
                  {/* 添加书签按钮 */}
                  <Button
                    variant="ghost"
                    size="icon"
          ...
          {/* 主题切换按钮 */}
          <div className="flex items-center bg-background/30 backdrop-blur-sm rounded-full px-1 sm:px-2 py-0.5 sm:py-1 border border-border/30 shadow-sm mr-1 sm:mr-2">
            <ThemeSwitcher />
          </div>
          ...
          {user ? (
            <div className="flex items-center gap-1 sm:gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="rounded-full border-primary/20 hover:border-primary/40 hover:bg-primary/5 p-1 sm:p-1.5 h-6 sm:h-7 w-6 sm:w-7 min-w-0">
            ...
            <Link href="/auth/signin">
              <Button size="sm" className="rounded-full shadow-sm hover:shadow-md transition-all duration-300 text-xs sm:text-sm h-6 sm:h-7 px-2 sm:px-3 py-0 min-w-0">
                登录
              </Button>
            </Link>
          )}
