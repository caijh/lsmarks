...
export function BookmarkCollectionCard({
  ...
  return (
    <Card className="overflow-hidden h-full flex flex-col relative group hover:shadow-xl transition-all duration-200 hover:-translate-y-1 border border-border/60 bg-card/65 backdrop-blur-sm shadow-md">
      {/* 卡片内主题渐变背景 */}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="h-7 w-7 sm:h-8 sm:w-8 rounded-full hover:bg-background/80 backdrop-blur-sm"
      ...
      <CardContent className="flex-grow p-3 sm:p-4 md:p-5 z-10 relative">
        ...
        <div className="absolute bottom-3 sm:bottom-4 md:bottom-5 left-3 sm:left-4 md:left-5 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none">
          <div className={`flex items-center justify-center w-8 sm:w-10 h-8 sm:h-10 rounded-md p-1 sm:p-1.5 border border-border/70 shadow-sm bg-background/90 backdrop-blur-sm relative overflow-hidden`}>
            {/* 图标内部渐变 */}
            ...
            <ExternalLink className={`h-5 w-5 sm:h-6 sm:w-6 ${is_public ? 'text-primary' : 'text-slate-600'} relative z-10`} />
          </div>
        </div>

        <div className="flex items-start gap-2 sm:gap-3 md:gap-4">
          <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 relative rounded-md flex items-center justify-center">
            {/* 图标容器背景效果 */}
            ...
            <div className="relative z-10 bg-background/90 backdrop-blur-sm rounded-md p-1.5 sm:p-2 border border-border/70 shadow-sm flex items-center justify-center w-full h-full overflow-hidden">
              {/* 图标内部渐变 */}
              ...
              {cover_url ? (
                ...
                <FolderOpen className={`h-5 w-5 sm:h-6 sm:w-6 ${is_public ? 'text-primary' : 'text-slate-600'} relative z-10`} />
              )}
            </div>
          </div>
          <div className="flex-grow min-w-0">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Badge
                      ...
                      className={`gap-0.5 sm:gap-1 h-4 sm:h-5 text-[10px] sm:text-xs ${is_public ? 'bg-primary/20 hover:bg-primary/30 text-gray-700 dark:text-primary-foreground border-primary/40 shadow-sm' : 'bg-slate-500/15 hover:bg-slate-500/25 text-slate-700 dark:text-slate-300 border-slate-500/30 shadow-sm'}`}
                    >
                      {is_public ? (
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Badge
                        variant="outline"
                        className="gap-0.5 sm:gap-1 h-4 sm:h-5 text-[10px] sm:text-xs bg-primary/15 border-primary/30 text-gray-700 dark:text-primary/90 shadow-sm"
                      >
            ...
            <div className="mt-2 sm:mt-3 px-2 sm:px-3 py-1.5 sm:py-2 bg-muted/50 rounded-md border border-border/60 shadow-inner relative overflow-hidden">
              {/* 描述框内主题渐变背景 */}
              ...
              <p className="text-xs sm:text-sm text-gray-600 dark:text-muted-foreground line-clamp-2 relative z-10">
