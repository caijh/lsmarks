...
CREATE OR REPLACE FUNCTION set_user_level(
    user_uuid_param VARCHAR(255),
    level_code_param VARCHAR(20)
)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $$
BEGIN
    -- 检查用户等级是否存在
    IF NOT EXISTS (SELECT 1 FROM user_levels WHERE level_code = level_code_param) THEN
        RAISE EXCEPTION '用户等级 % 不存在', level_code_param;
    END IF;

    -- 更新用户等级
    UPDATE users
    SET user_level = level_code_param,
        updated_at = NOW()
    WHERE uuid = user_uuid_param;
...
-- 注册限制功能已移除
-- 删除注册限制相关函数
DROP FUNCTION IF EXISTS check_registration_limit();
